...
describe FaqModule::RemoveService do
  ...
  describe '#call' do
    ...
    it "With invalid ID, receive error message" do
      ...
      response = @removeService.call()
      message = ['Não encontramos está questão, digite o número correto?', 'Não encontramos está questão, qual o número da questão?', 'Qual o id/número correto da questão?', 'Qual o número da questão?', 'Qual o número do faq?', 'Questão inválida ou inexistente, verifique o número do faq.']
